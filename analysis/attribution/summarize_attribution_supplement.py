#!/usr/bin/env python
r"""
Phase 9B v3 — SI statistical figures, performance figures, and gallery panels for the final 11M PathEOM-Net-R analysis.
...
def figure_confusion_matrix(counts_df: pd.DataFrame, out_png: Path):
    # Expected counts CSV rows: Negative, Positive, column_total
    use = counts_df[counts_df["actual_class"].isin(["Negative", "Positive"])].copy()
    mat = use[["pred_negative", "pred_positive"]].to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=(5.3, 4.4))
    im = ax.imshow(mat)
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Predicted negative", "Predicted positive"], rotation=12, ha="right")
    ax.set_yticks([0, 1])
    ax.set_yticklabels(["Actual negative", "Actual positive"])
    ax.set_title("Final 11M confusion matrix")
...
def figure_roc_curve(curve_df: pd.DataFrame, auc_value: float, out_png: Path):
    fig, ax = plt.subplots(figsize=(5.5, 5.0))
    ax.plot(curve_df["fpr"], curve_df["tpr"], linewidth=2, label=f"ROC-AUC={auc_value:.4f}")
    ax.plot([0, 1], [0, 1], linestyle="--", linewidth=1)
    ax.set_xlabel("False positive rate")
    ax.set_ylabel("True positive rate")
    ax.set_title("Final 11M ROC curve")
...
def figure_pr_curve(curve_df: pd.DataFrame, ap_value: float, out_png: Path):
    fig, ax = plt.subplots(figsize=(5.5, 5.0))
    ax.plot(curve_df["recall"], curve_df["precision"], linewidth=2, label=f"PR-AUC/AP={ap_value:.4f}")
    ax.set_xlabel("Recall")
    ax.set_ylabel("Precision")
    ax.set_title("Final 11M precision-recall curve")
...
def main():
    ...
    perf_json = attribution_dir / "summary_stats" / "final11m_performance_summary.json"
    conf_counts_csv = attribution_dir / "summary_stats" / "final11m_confusion_matrix_counts.csv"
    roc_points_csv = attribution_dir / "summary_stats" / "final11m_roc_curve_points.csv"
    pr_points_csv = attribution_dir / "summary_stats" / "final11m_pr_curve_points.csv"
    ...
    # Performance figures from final 11M prediction exports
    perf = json.loads(perf_json.read_text(encoding="utf-8")) if perf_json.exists() else {}
    if conf_counts_csv.exists():
        figure_confusion_matrix(pd.read_csv(conf_counts_csv), fig_dir / "figure_si_final11m_confusion_matrix.png")
    if roc_points_csv.exists():
        figure_roc_curve(pd.read_csv(roc_points_csv), float(perf.get("roc_auc", float("nan"))), fig_dir / "figure_si_final11m_roc_curve.png")
    if pr_points_csv.exists():
        figure_pr_curve(pd.read_csv(pr_points_csv), float(perf.get("pr_auc_average_precision", float("nan"))), fig_dir / "figure_si_final11m_pr_curve.png")
    ...
    manifest = {
        "version": "pcam_attribution_supplement_v1",
        ...
        "key_outputs": {
            ...
            "confusion_matrix_figure": str(fig_dir / "figure_si_final11m_confusion_matrix.png"),
            "roc_curve_figure": str(fig_dir / "figure_si_final11m_roc_curve.png"),
            "pr_curve_figure": str(fig_dir / "figure_si_final11m_pr_curve.png"),
        },
    ...
    (outdir / "pcam_attribution_supplement_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print("Done.")
