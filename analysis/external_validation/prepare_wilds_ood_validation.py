...
Purpose
-------
Create a local second held-out CAMELYON17-WILDS transfer-evaluation set for
PathEOM / MetaPath Paper 1 by streaming the mirror's validation split only.
...
This script streams the entire validation split to completion and saves every validation row.

Default output
--------------
  <repository>/data/raw/wilds_ood_validation

Files written
-------------
- camelyon17_wilds_ood_validation_x.h5
- camelyon17_wilds_ood_validation_y.h5
- camelyon17_wilds_ood_validation_metadata.csv
- wilds_ood_validation_manifest.json
...
Scientific role
---------------
This is a second held-out CAMELYON17-WILDS patch-level transfer split for a frozen
PCam-trained PathEOM evaluation. No balancing or class equalization is applied here.
The manifest records center counts so the saved mirror split remains auditable.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stream and save the CAMELYON17-WILDS OOD validation split."
    )
    parser.add_argument(
        "--outdir",
        default=str(Path(__file__).resolve().parents[2] / "data" / "raw" / "wilds_ood_validation"),
        help="Directory where OOD-validation HDF5 files and metadata will be written.",
    )
    parser.add_argument(
        "--hf-cache-dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Optional debugging cap. 0 means stream the full OOD validation split.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def ensure_output_paths(args: argparse.Namespace) -> Dict[str, Path]:
    ...
    paths = {
        "outdir": outdir,
        "x_h5": outdir / "camelyon17_wilds_ood_validation_x.h5",
        "y_h5": outdir / "camelyon17_wilds_ood_validation_y.h5",
        "metadata_csv": outdir / "camelyon17_wilds_ood_validation_metadata.csv",
        "manifest_json": outdir / "wilds_ood_validation_manifest.json",
    }
...
def main() -> int:
    ...
    try:
        ...
        pbar = tqdm(desc="Streaming CAMELYON17-WILDS OOD validation split", unit="row")
        for stream_index, row in enumerate(stream):
            ...
            counts[label] += 1
            center_key = str(metadata_value(row, "center"))
            slide_key = str(metadata_value(row, "slide"))
            center_counts[center_key] = int(center_counts.get(center_key, 0) + 1)
            slide_counts[slide_key] = int(slide_counts.get(slide_key, 0) + 1)
        ...
        failure = {
            ...
            "center_counts_before_failure": center_counts,
            "slide_counts_before_failure": slide_counts,
        }
        write_json(paths["outdir"] / "FAILED_wilds_ood_validation_download_error.json", failure)
        raise
    finally:
        try:
    ...
    manifest = {
        ...
        "split": SPLIT_NAME,
        "selection_rule": "stream every row in the Hugging Face validation split to completion; no balancing or subsampling",
    ...
    status(
        f"Completed OOD validation split. saved={saved_records:,}; negative={counts[0]:,}; "
        f"positive={counts[1]:,}; streamed={streamed_records:,}; centers={center_counts}"
    )
