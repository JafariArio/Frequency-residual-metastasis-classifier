...
Scientific role
---------------
This is a second held-out WILDS patch-level transfer stress test.
...
- No retraining
- No recalibration on the validation split
...
Default output directory:
  <repository>/results/wilds_ood_validation
...
- summary_stats\validation_center_counts.csv
...
- wilds_ood_validation_evaluation_manifest.json
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Frozen final 11M PathEOM-Net-R transfer evaluation on the CAMELYON17-WILDS OOD validation split."
    )
    parser.add_argument(
        "--subset-dir",
    ...
    parser.add_argument(
        "--x-h5",
        default="",
        help="Optional explicit x.h5 path. Defaults to subset-dir/camelyon17_wilds_ood_validation_x.h5.",
    )
    parser.add_argument(
        "--y-h5",
        default="",
        help="Optional explicit y.h5 path. Defaults to subset-dir/camelyon17_wilds_ood_validation_y.h5.",
    )
    parser.add_argument(
        "--metadata-csv",
        default="",
        help="Optional explicit metadata CSV. Defaults to subset-dir/camelyon17_wilds_ood_validation_metadata.csv if present.",
    )
    parser.add_argument(
        "--train-script",
...
def resolve_input_paths(args: argparse.Namespace) -> Dict[str, Path]:
    ...
    x_h5 = Path(args.x_h5) if args.x_h5 else subset_dir / "camelyon17_wilds_ood_validation_x.h5"
    y_h5 = Path(args.y_h5) if args.y_h5 else subset_dir / "camelyon17_wilds_ood_validation_y.h5"
    metadata_csv = Path(args.metadata_csv) if args.metadata_csv else subset_dir / "camelyon17_wilds_ood_validation_metadata.csv"
...
def save_curve_tables_and_figures(df: pd.DataFrame, stats_dir: Path, figs_dir: Path) -> None:
    ...
    plt.title("CAMELYON17-WILDS OOD validation ROC curve")
    plt.tight_layout()
    ...
    plt.title("CAMELYON17-WILDS OOD validation precision-recall curve")
    plt.tight_layout()
...
def save_confusion_figure(metrics: Dict[str, Any], figs_dir: Path) -> None:
    ...
    plt.title("CAMELYON17-WILDS OOD validation confusion matrix")
    plt.tight_layout()
...
def save_reliability_figure(df: pd.DataFrame, bins: int, figs_dir: Path) -> None:
    ...
    plt.title("CAMELYON17-WILDS OOD validation reliability diagram")
    plt.tight_layout()
...
def main() -> int:
    ...
    status("Opening CAMELYON17-WILDS OOD validation split.")
    ...
    pred_csv = out_paths["pred_dir"] / "validation_predictions.csv"
    ...
    metrics.update(
        {
            ...
            "ood_validation_split_dir": str(input_paths["subset_dir"]),
            "n_bootstrap": int(args.n_bootstrap),
            "ci_level": float(args.ci_level),
            "frozen_external_ood_validation_evaluation": True,
        }
    )
    ...
    if "center" in pred_df.columns:
        center_counts = pred_df.groupby("center").size().reset_index(name="count")
        center_counts.to_csv(
            out_paths["stats_dir"] / "validation_center_counts.csv",
            index=False,
        )
    else:
        center_counts = pd.DataFrame(columns=["center", "count"])
        center_counts.to_csv(
            out_paths["stats_dir"] / "validation_center_counts.csv",
            index=False,
        )
    ...
    manifest = {
        ...
        "center_counts_csv": str(out_paths["stats_dir"] / "validation_center_counts.csv"),
